...
/**
 * The HeartbeatChannel pairs with the HeartbeatMonitor. Both inject HEARTBEAT
 * messages into the underlying channel at a regular interval.
 */
export class HeartbeatChannel implements Channel {
     ...
     * offers special APIs.
     * @param interval A measurement in seconds specifying how long before the
     * channel should close. If you set the interval to 5 seconds, then the
     * monitor should set their timeout to 5 seconds; a 2x error margin.
     ...
     */
    constructor(channel: Channel, timeout: number) {
    ...
    private resetTimeout() {
        clearTimeout(this.timeoutId);
        this.timeoutId =
            setTimeout(this.beat, this.timeout * 1000) as any as number;
    }
}

/**
 * A HeartMonitor channel wraps another channel and executes a callback when the
 ...
 *
 * Note that any message recieved over the channel counts as a heartbeat. The
 * heartbeat only expires when no messages arrive within the timeout interval.
 * This means the other side only needs to send HEARTBEAT when no other message
 * has been sent within the last interval.
 */
export class HeartbeatMonitor implements Channel {
