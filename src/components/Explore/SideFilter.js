...
function SideFilter(props) {
    ...
    const fetchTraits = async () => {
      try {
        const response = await axios.get(`${contractConfig.NFT_API}/nfts/traits/${collection.contracts.base}`)
        ...
        {props.show ? <div className='ex-side-filter-container'>
          <Accordion open={open} toggle={toggle} stayOpen>
            <AccordionItem stayOpen>
              <AccordionHeader className='sf-header' targetId='collections'>COLLECTIONS</AccordionHeader>
              <AccordionBody accordionId='collections'>
                {!!(collections && collections.length) &&
                  collections.map((c, index) => (
                    <div key={index} className={`sf-item-wrapper ${c === collection ? 'selected-collection' : ''}`}>
                      <span
                        key={c.id}
            ...
            { collection?.label !== 'MetaHuahua' && attributes.map((attribute, index) => (
              <AccordionItem key={index} stayOpen>
                <AccordionHeader className='sf-header' targetId={attribute.type}>{titleize(attribute.type)}</AccordionHeader>
                <AccordionBody accordionId={attribute.type}>
                  {attribute.values.sort().map((option, i) => (
                    <div className='sf-item-wrapper' key={i}>
                      <input className="form-check-input" type="checkbox" id={`${option}-box`} onChange={() => handleSelections(option, attribute.type)} checked={selectedFilters[attribute.type]?.state?.includes(option)} />
                      <span className="form-check-label" htmlFor={`${option}-box`}>
                          {titleize(option)}
                      </span>
                    </div>
                  ))}
