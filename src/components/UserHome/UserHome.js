...
const UserHome = ({labeltoshow}) => {
  ...
  const removeWorldFromFav=async(worldid)=>{
    try {
      ...
      Notify.success({ title: "World has been removed from your Favourites", body: "World has been removed from your Favourites List", action: 'invite_success' });
      //updateList && updateList();
      setmakefavflag(false);
      setfavWorld(false);
    } catch (error) {
      console.log(error);
       Notify.error({ title: "Unsuccessful", body: "Error : Please try after sometime" });
  ...
  const addworldtofav=async(worldid)=>{
    try {
      ...
      setmakefavflag(true);
      setfavWorld(true);
      Notify.success({ title: "World has been added to Favourites", body: "World has been added to your Favourites List", action: 'invite_success' });
      //updateList && updateList();

      googleAnalyticsActions.initGoogleAnalytics(TRACKING_ID, 'Added Favourite', 'Favourite',);
    } catch (error) {
      console.log(error);
      if(error.response.status==500){
        Notify.error({ title: "Unsuccessful", body: "You have already made this world as favourite " });
      }
      else{
        Notify.error({ title: "Unsuccessful", body: "Error : Please try after sometime" });
  ...
  return (
    <div className='world-container'>
      <div className='world-detail-container home-container' style={{backgroundImage:`url(${bgImage})+no-repeat center center scroll`}}>
        ...
        {labeltoshow=="Promoted"?
          ...
          <img className='clan-logo' src={worldDataList.worldLogo} alt={worldDataList.worldName} />
          <span className='description-txt'>{worldDataList.longDescription}</span>
          <div className='world-cta-wrapper'>
            ...
            {(favflag==true||makefavflag==true)?<BookmarkIcon className='favicon' onClick={()=>removeWorldFromFav(worldDataList._id)}/>:<div><img className='favicon' src={staroutlined} alt={worldDataList.worldName} onClick={()=>addworldtofav(worldDataList._id)}/>
              <span className='btn-label' onClick={()=>addworldtofav(worldDataList._id)}>ADD TO FAVORITE</span></div>}
            </button>
          </div>
        </div>
        <div className='slidebuttondiv'>
             <span className='slidebutton'><Slidearrow className='expandbutton' onClick={() => {
          divRef.current.scrollIntoView({ behavior: "smooth" });
        }}/></span>
      </div>
      </div>
      <div className='socialcontent' ref={divRef}>
      <SocialFeedPostWorldGame/>
      </div>
    </div>
  )
}
