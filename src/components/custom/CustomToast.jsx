  ...
  return (
    <div style={{ ...styles.container }}>
      ...
      <div style={{ ...styles.middle }} onClick={() => onClick && onClick()}>
        {type === 'ERROR' ?
          <span style={{ ...styles.headerTxt, ...styles.headerError }}>{title}</span> :
          <span style={{ ...styles.headerTxt, ...styles.headerSuccess }}>{title}</span>
        }
        {children}
      </div>
      ...
      <span style={{ ...styles.right }}>
        <span style={{ ...styles.timeStamp }}>Now</span>
          <img style={{ cursor: 'pointer' }} src={CloseIcon} height={10} width={10} alt="Cross" onClick={onClose}/>
      </span>
    </div>
  );
}
...
const getToast = (title = "Notification", content, meta = {}, type) => {
  toast(
    ({ closeToast }) => (
      <ToastContent onClose={closeToast} title={title} type={type} logo={meta?.logo} onClick={meta?.handleClick}>{content}</ToastContent>
    ),
    {
      closeButton: false,
